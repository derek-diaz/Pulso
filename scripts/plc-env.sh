...
if [[ ! -f "${LIBPLCTAG_PKG_CONFIG_DIR}/libplctag.pc" ]]; then
  ...
  if [[ ! -f "${LIBPLCTAG_PKG_CONFIG_DIR}/libplctag.pc" ]]; then
    ...
    echo "Run bash scripts/setup-libplctag.sh, or set PULSO_LIBPLCTAG_AUTO_SETUP=1 and rerun the PLC-enabled app." >&2
    return 1
  fi
fi
...
export DYLD_LIBRARY_PATH="${LIBPLCTAG_LIB_DIR}${DYLD_LIBRARY_PATH:+:${DYLD_LIBRARY_PATH}}"
